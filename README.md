...
### Current fork compared with upstream Rethlas

Here, “upstream” means `origin/main` at commit `887cc4642763`. The comparison
is architectural: stronger orchestration does not imply that the current fork
solves more mathematics, and a successful control plane is not a verified
proof. “Current legacy” means the current hardened `codex exec` path with
`RETHLAS_HOTJOIN_RUN_ID` unset. It is not the historical upstream
implementation: only its cadence and context-guard policies are disabled.

| Area | Upstream Rethlas | Current legacy mode | Current hot-join mode |
| --- | --- | --- | --- |
| Core workflow | Two Codex agents: generation writes a blueprint and verification checks it. | Keeps the two roles, but uses the current safe-route protocol, hardened MCP runtime, and receipt-gated verifier without an owner scheduler. | Adds the owner-side control plane, scheduled route critic, Guardian, and durable ledger to the current generation/verifier roles. |
| Generation sessions | Iteration zero starts one Codex session; later iterations resume that same session until `MAX_ITERATIONS`. | Starts a fresh ephemeral `codex exec` session per iteration, bounded by `MAX_ITERATIONS`. | Uses durable app-server thread epochs and bounded fresh-thread handoffs inside an absolute route cycle. |
| Parallel proof search | `$recursive-proving` starts one child per plan; children may recursively spawn and all agents may write shared memory. | If no complete candidate exists, the reasoning contract requires exactly three scope-disjoint, context-free children. They cannot spawn or write shared memory, but there is no durable Guardian proof-lane enforcement. | Continuously enforces at most three concurrent proof lanes. The no-candidate initial fanout still has exactly three children; a ready candidate skips it. The host freezes and later drains the admitted lane set. |
| Search policy | Initializes memory before reasoning and directs nontrivial subgoals toward arXiv/web retrieval. | Protects initial route design from retrieval and memory writes; later retrieval is bounded and optional. | Uses the same protected reasoning policy and additionally binds released checkpoint visibility to host cadence cutoffs. |
| Memory and checkpoints | Direct JSONL `memory_append` plus mutable `branch_update` records. | Uses content-addressed checkpoint batches with local commit receipts, but no cadence-database admission authority. | Adds immutable host admission receipts, event provenance, review cutoffs, and separate reasoning/control publication classes. |
| Route governance | Adaptive prompt loop with no absolute independent route-review cadence. | Has no official route review: the review policy is disabled, so there is no scheduled critic, yellow/red transition, or `route_frozen`. The final proof verifier remains available. | Independent route reviews at T+60m and T+120m, yellow fatal-doubt restriction, effective-red logic, T+147m close, and T+150m hard stop. |
| Work at review boundaries | No protocol for preserving a child that is mid-turn when review or shutdown occurs. | Has no scheduled review boundary and therefore makes no host-enforced drain or partial-report guarantee. | Root-managed cooperative drain, five-minute grace, straggler-only interruption, sealed complete/partial reports, and untrusted-scratch rehydration. |
| Context rollover | Continues the same transcript/session. | Reconstructs each fresh iteration from authoritative local files and bounded memory; there is no durable cross-thread handoff contract. | Uses a content-addressed handoff into a strictly newer thread epoch; compaction and transcript length do not count as progress. |
| Verification | A writable verifier agent creates `verification.json`; the runner trusts the resulting workspace file and uses `--dangerously-bypass-approvals-and-sandbox`. | Uses fresh isolated read-only verifier sessions, schema-constrained direct JSON, adaptive proof context, and atomic publication with an external receipt. | Uses the same verifier, while the ledger also binds review, targeted-verification, and continuation authority. |
| Runtime and process safety | Direct Codex/FastMCP execution with limited host-side lifecycle accounting. | Uses the official MCP SDK, attested runtime snapshots, isolated MCP roles, an ephemeral workspace-write Codex sandbox, and restricted shell environment. | Adds capability-scoped app-server interfaces, append-only SQLite state, process-group Guardian, and fail-closed recovery. |
| Human and cost control | No durable hot-join, advisor, or owner-yield state machine. | No hot-join owner messages or unfinished owner-wait state; an unsolved run returns unverified. | Audited owner hot-join, evidence-bound advisor requests, hash-bound cost policy, and explicit paid-continuation admission. |
| Completion authority | Presence of `blueprint_verified.md` is the runner stop condition. | Only `verify_blueprint_service` plus its matching trusted external receipt can finish a run. | Uses the same proof authority; reviews, control-plane success, stale files, and drafts remain non-authoritative. |
| Validation surface | Upstream documents no repository-wide pytest result and tracks one Python test script. | Covered by the current 1,176-test and 56-subtest repository suite and the publication smoke. | Covered by that same suite plus real app-server, Guardian, review, fresh-epoch, and cooperative-drain smokes. |
| Main tradeoff | Smallest and easiest to understand, but relies heavily on model compliance and has weak crash/interface provenance. | Cheaper and operationally simpler than hot-join while retaining most mathematical and verifier hardening; it gives up durable timing, mid-turn preservation, and owner control. | Most auditable and resistant to unsafe continuation, but consumes more model work, is substantially harder to operate, and depends on experimental Codex app-server v2. |
