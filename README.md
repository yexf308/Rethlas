...
| Area | Upstream Rethlas | Current fork |
| --- | --- | --- |
| Core workflow | Two Codex agents: generation writes a blueprint and verification checks it. | Retains those roles and adds an owner-side control plane, scheduled route critic, Guardian, and receipt-gated publication. |
| Generation sessions | Iteration zero starts one Codex session; later iterations resume that same session until `MAX_ITERATIONS`. | Legacy mode uses fresh sessions. Optional hot-join mode uses durable app-server epochs and bounded fresh-thread handoffs. |
| Parallel proof search | `$recursive-proving` starts one child per plan; children may recursively spawn and all agents may write shared memory. | At most three proof lanes may run concurrently. If protected route design finds no complete candidate, the initial fanout contains exactly three scope-disjoint, context-free children; a ready candidate skips that fanout. Children cannot spawn, write shared memory, switch routes, or create a fourth direction; the root is the sole merger. |
| Search policy | Initializes memory before reasoning and directs nontrivial subgoals toward arXiv/web retrieval. | Protects the initial route-design phase from retrieval and memory writes; later retrieval is bounded and optional rather than evidence of progress. |
| Memory and checkpoints | Direct JSONL `memory_append` plus mutable `branch_update` records. | Content-addressed checkpoint batches, host admission receipts, immutable event provenance, and separate reasoning/control publication classes. |
| Route governance | Adaptive prompt loop with no absolute independent route-review cadence. | One absolute 150-minute cycle with independent reviews at T+60m and T+120m, yellow fatal-doubt restriction, effective-red logic, and `route_frozen` termination. |
| Work at review boundaries | No protocol for preserving a child that is mid-turn when review or shutdown occurs. | Root-managed cooperative drain, five-minute grace, straggler-only interruption, sealed complete/partial reports, and untrusted-scratch rehydration. |
| Context rollover | Continues the same transcript/session. | Bounded content-addressed handoff into a strictly newer thread epoch; automatic compaction and transcript length do not count as progress. |
| Verification | A writable verifier agent creates `verification.json`; the original runner trusts the resulting workspace file and uses `--dangerously-bypass-approvals-and-sandbox`. | Fresh isolated read-only verifier sessions return schema-constrained direct JSON, expand proof context adaptively, and publish only through an atomic external receipt. |
| Runtime and process safety | Direct Codex/FastMCP execution with limited host-side lifecycle accounting. | Official MCP SDK compatibility, attested runtime snapshots, capability-scoped interfaces, append-only SQLite ledger, process-group Guardian, and fail-closed recovery states. |
| Human and cost control | No durable hot-join, advisor, or owner-yield state machine. | Audited owner hot-join, evidence-bound advisor requests, hash-bound cost policy, and explicit paid-continuation admission. |
| Completion authority | Presence of `blueprint_verified.md` is the runner stop condition. | Only `verify_blueprint_service` plus its matching trusted external receipt can finish a run; stale files, drafts, and review success are non-authoritative. |
| Validation surface | Upstream documents no repository-wide pytest result and tracks one Python test script. | The current checked suite has 21 Python test files; 1,176 tests and 56 subtests passed, with one expected skip, plus real app-server and publication smokes. |
| Main tradeoff | Smaller and easier to understand, but relies heavily on model compliance and has weak crash/interface provenance. | More auditable and resistant to unsafe continuation, but substantially larger, operationally harder, and dependent on the experimental Codex app-server v2 interface. |
