...
| Parallel proof search | `$recursive-proving` starts one child per plan; children may recursively spawn and all agents may write shared memory. | Exactly three scope-disjoint, context-free proof children start in one fanout. Children cannot spawn, write shared memory, switch routes, or create a fourth direction; the root is the sole merger. |
